...
def main(argv):
	...
	while i <= n:
		...
		for commit in range(0, num_commits):
			subprocess.call("echo '" + curdate + str(randint(0, 1000000)) +"' > b45.sh; git add b45.sh; GIT_AUTHOR_DATE='" + curdate + "' GIT_COMMITTER_DATE='" + curdate + "' git commit -m 'encrypted'; git push;", shell=True)
			sleep(.5)
		i += 1
	subprocess.call("git rm b45.sh; git commit -m 'delete'; git push;", shell=True)
